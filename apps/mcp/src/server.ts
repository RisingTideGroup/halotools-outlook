  ...
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        ...
        "- Embeddings (FaultVectorScore): fvsSearchMethod is the vector BACKEND (0=Halo internal store, 1=Azure AI Search, 2=OpenSearch) — filter to the tenant's ACTIVE one = Control4.rTicketEmbeddingsDB (old backends leave stale rows; NULL-method rows are garbage). FVSuse=0 = ticket↔ticket, FVSuse=1 = ticket↔KB (FVSSimiliarfaultid is then a KBENTRY.id). Score minimums live in Control4.rvectorscoreminimum / rvectorscoreknowledgeminimum.\n" +
