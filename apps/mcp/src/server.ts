  ...
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        ...
        "- Time/billability: worked hours = ACTIONS.timetaken; billable = timetaken where ActIsBillable=1 (ActionChargeHours/ActionNonChargeHours are unreliable/empty in many tenants — don't sum them). Action work date = COALESCE(Whe_, ActionArrivalDate, ActionDateCreated) since ActionDateCreated can be backdated.\n" +
        ...
        "- Resourcing sources: APPOINTMENT = booked calendar (APStartDate, exclude APdeleted/APAllDayEvent); HOLIDAYS = leave (Hduration, HTechnicianID); TimesheetEvent = clock/shift hours (often sparse).\n\n" +
