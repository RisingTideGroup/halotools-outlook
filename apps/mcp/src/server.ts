  ...
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        ...
        "TOOL FAMILIES:\n" +
        "- Ad-hoc analysis: exploreSchema (START HERE for unfamiliar data — discover tables/columns and sample real rows so you understand the schema before querying), runSql (highest-leverage — SELECT via Report Center; read its 7 rules) and listReports (check for an existing saved report FIRST). haloApiRaw is the REST escape hatch for writes / non-SQL endpoints.\n" +
