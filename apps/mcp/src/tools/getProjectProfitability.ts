...
export function registerGetProjectProfitability(server: McpServer): void {
  server.registerTool(
    "getProjectProfitability",
    {
      ...
      description:
        "Per-project profitability. Projects are refilled PREPAY BLOCKS, not fixed-fee: the budget is prepayPurchasedHours (PREPAYHISTORY top-ups on the contract), not the stale estimate. Auto-detects the billing model (retainer prepay → T&M ActionChargeAmount → fixed/internal) and returns revenue, delivered hours, billableHours, prepayPurchasedHours/prepayConsumedHours, soldRate (revenue/purchased hrs), effectiveRate (revenue/delivered hrs), unchargedHours (billable time billed NEITHER via prepay draw-down NOR a direct charge amount = genuinely leaked labour) + unchargedValue, labour cost with coverage %, gross margin (reliable only when costCoveragePct≥80), and overServiced (delivered > purchased block). All amounts in the home currency (returned in `currency`). Effective/sold rate is the robust profitability proxy when cost coverage is low.",
      inputSchema,
    },
