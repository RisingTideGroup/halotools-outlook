...
export function registerGetRecurringContractProfitability(server: McpServer): void {
  server.registerTool(
    "getRecurringContractProfitability",
    {
      ...
      description:
        "Are we profitable on our managed-services agreements? Compares monthly recurring revenue against the support effort delivered for it, at one of two grains (groupBy): per CLIENT (default — whole-client margin) or per CONTRACT (revenue tied to each contract via the generated recurring invoice line, INVOICEDETAIL.IDCHID; labour via ACTIONS.AContractId). Returns recurringRevenueMonthly (actual recurring invoiced in the latest complete calendar month — never TTM/12; use getMrrSnapshot.recentMonths for the trailing read), supportHoursMonthly, billable share, the techs who logged the time (topTechs, with hours + best-effort cost), and a margin where agent-cost data allows. At contract grain, unattributedRevenueMonthly holds recurring revenue on lines with no contract so the rows reconcile to total MRR. " +
