  ...
  server.registerTool(
    "runSql",
    {
      ...
      description: [
        ...
        "- Reactive support = ticket types whose ITIL category qualifies: `FAULTS.RequestType` carries the ITIL code directly (== `REQUESTTYPE.RTRequestType` via `RequestTypeNew → RTid`). Use Incident + Service Request: `RequestType in (1,3)`. EXCLUDE Advice/Other (21) — the quick-time/admin/recordings catch-all that can be the bulk of tickets and distorts every metric. Project work = 22/23/24; Problem (4) is its own ITIL class. Do NOT use RTIsProject/RTIsOpportunity flags for reactive (they leave 21 in). Tenants add custom ticket-type IDs but they roll up to these ITIL codes — confirm the mapping once.",
