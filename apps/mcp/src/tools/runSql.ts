  ...
  server.registerTool(
    "runSql",
    {
      ...
      description: [
        ...
        "- MTTR: wall-clock `datediff(minute, dateoccured, datecleared)` includes customer-wait/hold time. For WORKING-hours use the scalar function `dbo.Fn_GetWorkingHours_datetimes(@start, @end, @slaid, @faultid, @timezone_override)` — e.g. `dbo.Fn_GetWorkingHours_datetimes(f.dateoccured, f.datecleared, f.slaid, f.faultid, null)`.",
