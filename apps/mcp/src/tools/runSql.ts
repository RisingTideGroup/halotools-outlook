  ...
  server.registerTool(
    "runSql",
    {
      ...
      description: [
        ...
        "- MRR / recurring revenue = the ACTUAL generated recurring invoices, not the schedule's nominal amount (it drifts). Recurring master = `INVOICEHEADER` where `IHid<0` (`IHisRecurringInvoice=1`); each period it generates a real invoice `IHid>0` with `IHrecurringInvoiceId` = the master. MRR = trailing-12-month net (`sum(INVOICEDETAIL.IDNet_Amount)`) of those generated invoices ÷ 12. Cadence = `IHPeriodEndDate − IHPeriodStartDate`. (STDREQUEST holds ALL schedulers — tickets, runbooks, templates — `StdPeriod` is the nominal cadence; trust the invoice for the amount.)",
        ...
        "- Embeddings / ticket similarity = `FaultVectorScore` (FVSfaultid → FVSSimiliarfaultid, FVSScore cosine). `fvsSearchMethod` is the vector BACKEND (0=Halo internal store, 1=Azure AI Search, 2=OpenSearch) — ALWAYS filter to the tenant's active one = `(select max(rTicketEmbeddingsDB) from Control4)` (old backends leave stale rows; NULL-method rows score garbage at 1.0). `FVSuse`: 0 = ticket↔ticket (FVSSimiliarfaultid is a FAULTS.faultid), 1 = ticket↔KB (it's a KBENTRY.id). Score minimums: Control4.rvectorscoreminimum (tickets) / rvectorscoreknowledgeminimum (KB).",
