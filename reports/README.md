...
- **`getProjectPortfolio` / `getProjectProfitability` / `getResourceForecast`** — project oversight. Portfolio health (% complete via child tasks, hours, age); **auto-detected-billing-model profitability** (retainer prepay → T&M `ActionChargeAmount` → fixed/internal; revenue + effective rate + pay-type-adjusted cost with a coverage flag + over-servicing); and forward resource load (booked `APPOINTMENT` hours vs weekly capacity → over-allocated / bench). Project gotchas: money links via `FAULTS.fcontractid`; main project = `fmainprojectid` NULL/0/self; deadlines & money-budget fields empty; agent cost (`ucostPrice`, pay-type-adjusted) only ~partly populated.
